...
export async function deleteTag(params: { tagId: string, path: string }) {
  try {
    connectToDatabase();
...
export async function updateTag(params: { tagId: string, name: string, description: string, path: string }) {
  try {
    connectToDatabase();

    const { tagId, name, description, path } = params;

    await Tag.findByIdAndUpdate(tagId, { name, description });
